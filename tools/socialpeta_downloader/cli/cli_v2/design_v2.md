...
- Hệ thống gọi một cửa sổ GUI Explorer (ví dụ thông qua thư viện `tkinter.filedialog` hoặc select folder trong nền) để người dùng chọn trực tiếp thư mục lưu trữ Video Workspace. Điều này giúp tránh việc phải gõ tay đường dẫn phức tạp.
...
3. **Cửa sổ chọn thư mục (Folder Picker)**:
   - Sử dụng `tkinter` (mặc định đi kèm Python) thông qua `filedialog.askdirectory` chạy ngầm không hiển thị cửa sổ chính để người dùng chọn thư mục lưu một cách chuyên nghiệp.
