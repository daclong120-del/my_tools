...
class SnifferService:
    ...
    def _navigate_to_page(self, page, page_num: int) -> bool:
        """
        Helper để điều hướng đến một trang số page_num cụ thể.
        """
        try:
            page.keyboard.press("End")
            time.sleep(1.0)
        ...
        if self.context and self.context.utils_service:
            if page_num <= 5:
                success = self.context.utils_service._click_page_button(page, page_num)
            else:
                success = self.context.utils_service._jump_to_page(page, page_num)
            return success
        return False
    ...
    def run_scrape_current_page_cli(self, argv: Optional[list] = None, csv_path: Optional[str] = None, port: int = 9222) -> None:
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                success_temp = self._navigate_to_page(page, temp_page)
                if not success_temp:
                ...
                with page.expect_response(lambda r: "/creative/list" in r.url or "/creative-rank/list" in r.url, timeout=20000) as response_info:
                    success_back = self._navigate_to_page(page, current_page)
                    if not success_back:
                ...
                body = response.json()
                raw_items = self._recursive_find_creatives(body)
                if raw_items:
                    print(f"[📡] Bắt được {len(raw_items)} quảng cáo thô từ API phản hồi.")
                    for raw in raw_items:
                        parsed = self._parse_creative_item(raw)
                        parsed_items.append(parsed)
                else:
                    print("[-] Không tìm thấy quảng cáo nào trong phản hồi API.")
                    
                # Bước 4: Click chính xác các icon YouTube để lấy link thực tế
                print("\n[*] Bước 4: Tiến hành quét và click các icon YouTube trên trang hiện tại...")
                clicked_results = self.context.youtube_service.click_and_extract_youtube_from_page(page)
                
                if clicked_results:
                    print(f"[+] Trích xuất được {len(clicked_results)} link YouTube từ thao tác click.")
                    
                    # Bước 5: Khớp nối link YouTube với dữ liệu quảng cáo từ API...
                ...
                if parsed_items:
                    ...
                    for item in parsed_items:
                        self.context.append_to_custom_csv(csv_path, item)
                ...
                if raw_items:
                    ...
                    for raw in raw_items:
                        ...
                        if ad_id and ad_id not in scraped_ids:
                            ...
                            self.context.append_to_custom_csv(csv_path, parsed)
