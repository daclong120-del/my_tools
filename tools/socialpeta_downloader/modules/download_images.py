...
def download_image(url, output_path):
    """
    Download an image directly from the URL.
    """
    headers = {
...
def process_row(index, item, output_dir, total_images):
    """
    Worker function to process and download a single image creative.
    """
    ...
    print(f"[{index}/{total_images}] Downloading: {final_filename}...")
    success = download_image(img_url, final_path)
    if success:
...
def main():
    ...
    output_dir = os.path.join(script_dir, "download_images")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        futures = {
            executor.submit(process_row, index, item, output_dir, total_images): item 
