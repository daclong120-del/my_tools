...
def main():
    ...
    output_file = sys.argv[2] if len(sys.argv) > 2 else os.path.join(modules_dir, "scraped_creatives_cdn_video_only.csv")
    
    core = SocialPetaDownloaderCore(skip_db_init=True)
    core.downloader_service.run_filter_cdn_video_creatives_cli(input_file=input_file, output_file=output_file)
